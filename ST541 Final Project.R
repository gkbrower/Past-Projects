...
## Cleaning Data With Any Null Values from Our Dataset
...
final_data$MntWines <- final_data$MntWines+0.00001 # adjusting for possible transformation in the future
final_data$MntMeatProducts <- final_data$MntMeatProducts+0.00001
final_data$Dt_Customer <- as.Date(final_data$Dt_Customer, format="%d-%m-%Y") # making the date value numeric instead of categorical
...
pchisq(469.09, 2203, lower.tail = FALSE)


# We also used a poisson regression to attempt to predict the number of in-store purchases. There was not over-dispersion, so a quasipoisson model was not needed.
...
# Plotting significant predictors
par(mfrow=c(3,3))
plot(MntWines~Year_Birth+Income+Dt_Customer+MntMeatProducts+MntSweetProducts+MntGoldProds+
...
fish_data <- final_data[-c(1634, 1879, 21, 672, 1954, 154),]
par(mfrow=c(2,2))

m3.1 <- lm((fish_data$MntFishProducts)~., data=fish_data)
fish_model1 <- step(m3.1, direction="both", data = fish_data)
summary(fish_model1)
plot(fish_model1)

m3.2 <- lm((fish_data$MntFishProducts)^(1/2)~., data=fish_data)
fish_model2 <- step(m3.2, direction="both", data = fish_data)
summary(fish_model2)
plot(fish_model2)


fruit_data <- final_data
par(mfrow=c(2,2))

m4.1 <- lm(fruit_data$MntFruits~., data=fruit_data)
fruit_model1 <- step(m4.1, direction="both", data = fruit_data)
summary(fruit_model1)
plot(fruit_model1)

m4.2 <- lm((fruit_data$MntFruits)^(1/2)~., data=fruit_data)
fruit_model2 <- step(m4.2, direction="both", data = fruit_data)
summary(fruit_model2)
plot(fruit_model2)

par(mfrow=c(2,2))

m5.1 <- lm(final_data$MntGoldProds~., data=final_data)
gold_model1 <- step(m5.1, direction="both", data = final_data)
summary(gold_model1)
plot(gold_model1)

m5.2 <- lm((MntGoldProds)^(1/2)~., data=final_data)
gold_model2 <- step(m5.2, direction="both", data = final_data)
summary(gold_model2)
plot(gold_model2)


sweet_data <- final_data[-c(1637, 1882, 1634, 672, 21, 154),]
par(mfrow=c(2,2))

m6.1 <- lm(sweet_data$MntSweetProducts~., data=sweet_data)
sweet_model1 <- step(m6.1, direction="both", data = sweet_data)
summary(sweet_model1)
plot(sweet_model1)

m6.2 <- lm((sweet_data$MntSweetProducts)^(1/2)~., data=sweet_data)
sweet_model2 <- step(m6.2, direction="both", data = sweet_data)
summary(sweet_model2)
plot(sweet_model2)
...
# Plotting significant predictors
par(mfrow=c(2,3))
plot(MntMeatProducts~Income+Dt_Customer+Recency+MntWines+MntFishProducts+MntSweetProducts+MntGoldProds+
